...
function synthesizeSegment(text, voiceId, apiKey, speed = 1.0) {
...
function parseDialogueSegments(text, defaultGender = 'female') {
...
async function run() {
  ...
  // Load all 10 exam papers using tsx or pre-compiled bundle
  const { getExamRegistry } = require(path.join(__dirname, "../dist/assets/index-BwhsV3vI.js") || {});

  // Fallback to dynamic loading if compiled file name shifts
  let examRegistryFn = getExamRegistry;
  if (!examRegistryFn) {
    console.log("Locating compiled asset with getExamRegistry...");
    const fs = require("fs");
    const assetsDir = path.join(__dirname, "../dist/assets");
    const files = fs.readdirSync(assetsDir);
    const indexFile = files.find(f => f.startsWith("index-") && f.endsWith(".js"));
    if (indexFile) {
      const mod = require(path.join(assetsDir, indexFile));
      examRegistryFn = mod.getExamRegistry;
    }
  }

  const papers = examRegistryFn ? examRegistryFn().slice(0, 10) : [];
  console.log(`Loaded ${papers.length} exam papers from registry.`);
  ...
  for (let pIdx = 0; pIdx < papers.length; pIdx++) {
    ...
    for (let qIdx = 0; qIdx < listeningSection.questions.length; qIdx++) {
      ...
      const speakingRate = q.speakingRate || 1.0;
