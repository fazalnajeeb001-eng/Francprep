...
for (let p = 1; p <= 10; p++) {
  ...
  qs.forEach(q => {
    ...
    frenchLeakWords.forEach(w => {
      if (qEn.includes(w)) {
        console.log(`[P${p}Q${qNum}] qEn leak: "${w}" in "${q.questionPromptEnglish}"`);
      }
    });
    ...
    q.optionsEnglish?.forEach((opt: string, optIdx: number) => {
      ...
      frenchLeakWords.forEach(w => {
        if (oEn.includes(w)) {
          console.log(`[P${p}Q${qNum}] optEn[${optIdx}] leak: "${w}" in "${opt}" (fr: "${q.options[optIdx]}")`);
        }
      });
    });
    ...
    frenchLeakWords.forEach(w => {
      if (passEn.includes(w)) {
        console.log(`[P${p}Q${qNum}] passEn leak: "${w}" in "${q.passageEnglish}"`);
      }
    });
  });
}
