...
console.log("================================================================================");
console.log("🇨🇦 TEF CANADA COMPRÉHENSION ORALE: PAPER 3 FULL FORENSIC INTEGRATION TEST");
console.log("================================================================================");

const registry = getExamRegistry();

// 1. Find TEF Paper 3 Exam and Practice papers
const examPaper3 = registry.find((p) => p.id === "tef-canada-official-exam-paper-3");
const pracPaper3 = registry.find((p) => p.id === "tef-canada-practice-paper-3");

if (!examPaper3) {
  console.error("❌ FAILED: 'tef-canada-official-exam-paper-3' not found in registry.");
  process.exit(1);
}
if (!pracPaper3) {
  console.error("❌ FAILED: 'tef-canada-practice-paper-3' not found in registry.");
  process.exit(1);
}

console.log("✓ Found TEF Canada Official Exam Paper 3 & Guided Practice Paper 3.");

// 2. Audit Section Duration & Total Items
const examListening = examPaper3.sections.find((s) => s.type === "COMPREHENSION_ORALE");
const pracListening = pracPaper3.sections.find((s) => s.type === "COMPREHENSION_ORALE");

if (!examListening || examListening.totalQuestions !== 40 || examListening.durationMins !== 40) {
  console.error("❌ FAILED: Exam Paper 3 Compréhension Orale must be 40 questions and 40 minutes.", examListening);
  process.exit(1);
}

console.log(`✓ Section Verification: 40 Questions / 40 Minutes confirmed (Total Exam Duration: ${examPaper3.totalDurationMins} Mins).`);

// 3. Question Item Integrity Audit across all 40 questions
console.log("\n[Audit 1] Verifying all 40 Authentic Questions for Paper 3...");
examListening.questions.forEach((q, idx) => {
  ...
  if (!q.id.startsWith("tef-p3-co-")) {
    console.error(`❌ Question ${qNum} has unexpected id pattern: ${q.id}`);
    process.exit(1);
  }
});
console.log("✓ All 40 questions verified: valid French/English text, 4 options, transcripts, correct indices, and tef-p3-co IDs.");
...
console.log("\n[Audit 3] Verifying Dual-Mode Pedagogy & Zero-Leak Lockdown for Paper 3...");
examListening.questions.forEach((q, idx) => {
  const qNum = idx + 1;
  if (q.trapAlert !== undefined || q.audioCoach !== undefined) {
    console.error(`❌ EXAM MODE LEAK on Paper 3 Q${qNum}: trapAlert or audioCoach is exposed in exam mode!`);
    process.exit(1);
  }
});
...
pracListening.questions.forEach((q, idx) => {
  ...
  if (!q.trapAlert || q.trapAlert.length === 0) {
    console.error(`❌ PRACTICE MODE: Paper 3 Q${qNum} missing trapAlert!`);
    process.exit(1);
  }
  if (!q.audioCoach || q.audioCoach.length === 0) {
    console.error(`❌ PRACTICE MODE: Paper 3 Q${qNum} missing audioCoach!`);
    process.exit(1);
  }
  if (!q.explanation || q.explanation.length === 0) {
    console.error(`❌ PRACTICE MODE: Paper 3 Q${qNum} missing detailed explanation!`);
    process.exit(1);
  }
  if (!q.detailedExplanationEn || q.detailedExplanationEn.length === 0) {
    console.error(`❌ PRACTICE MODE: Paper 3 Q${qNum} missing English detailed explanation!`);
    process.exit(1);
  }
  // Anti-spoil test: trap alert must not state 'Option A' or 'Option B'
  if (/Option\s+[A-D]\s+est\s+la\s+bonne/i.test(q.trapAlert)) {
    console.error(`❌ PRACTICE MODE SPOIL on Paper 3 Q${qNum}: trapAlert contains answer spoiler!`);
    process.exit(1);
  }
});
...
console.log("\n╔══════════════════════════════════════════════════════════════════════════════════╗");
console.log("║     🎉 TEF CANADA COMPRÉHENSION ORALE (PAPER 3): 100% CERTIFIED & PASSED!        ║");
console.log("╚══════════════════════════════════════════════════════════════════════════════════╝\n");
