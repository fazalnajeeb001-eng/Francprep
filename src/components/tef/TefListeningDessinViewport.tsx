...
export const TefListeningDessinViewport: React.FC<TefListeningDessinViewportProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto w-full space-y-4 px-2 sm:px-4 py-2">
        ...
        <div className="p-3 sm:p-3.5 rounded-xl border border-amber-500/40 bg-amber-950/30 text-amber-100 shadow-sm space-y-2.5">
          <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
            <div className="flex items-center gap-2 font-mono font-bold text-amber-400">
              ...
              <span>👑 SUPER-ADMIN QA INSPECTOR (Mode Examen — Section A Dessins)</span>
            </div>
          ...
          <div className="flex items-center justify-between text-[11px] text-amber-200/80 font-mono border-t border-amber-500/20 pt-1.5">
            <span>
              Locuteurs : <strong>{(currentQ as any).speakers ? (currentQ as any).speakers.join(", ") : "Dialogue court"}</strong> • Scène attendue : <strong>Dessin {String.fromCharCode(65 + currentQ.correctIndex)}</strong>
            </span>
        ...
        {currentQ.options.map((_, idx) => {
          ...
          if (isChecked) {
            ...
            if (isChosen) {
              ...
              cardStyle = "border-amber-500/80 ring-2 ring-amber-500/40 bg-amber-500/5 dark:bg-amber-950/20";
              badgeElement = (
                <span className="text-[10px] font-mono font-bold uppercase tracking-wider bg-amber-500/20 text-amber-800 dark:text-amber-200 border border-amber-500/40 px-2 py-0.5 rounded flex items-center gap-1">
      ...
      {/* ─── 6. PRACTICE MODE OR ADMIN QA EXPANDED PEDAGOGICAL GUIDANCE ─── */}
