...
export function FlagIcon({ code, className = "w-7 h-5", glow = false }: FlagIconProps) {
  ...
  return (
    <div className={`relative inline-flex items-center justify-center overflow-hidden shrink-0 select-none ${className}`}>
      {getSvgContent()}
      {/* 3D Glossy Light Reflection Sheen */}
      ...
      <div className="absolute inset-0 ring-1 ring-black/15 ring-inset rounded-[inherit] pointer-events-none" />
    </div>
  );
}
