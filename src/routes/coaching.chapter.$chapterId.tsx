...
const chapters: Record<string, {
  id: string; title: string; level: string; module: string; objective: string;
  estTime: string; lessons: { number: number; title: string; skill: string; time: string; id: string }[];
}> = {
  "a1-1-1-1": {
    id: "a1-1-1-1",
    level: "A1",
    ...
    objective: "Greet people appropriately, introduce yourself, and choose between formal and informal address.",
    estTime: "3–3.5 hours",
    lessons: [
  ...
  if (!chapter) {
    return (
      <div className="min-h-screen dark:bg-[#070B17] bg-gray-50 flex items-center justify-center p-4">
        <div className="text-center"><p className="text-4xl mb-4">📚</p><p className="text-sm dark:text-gray-400 text-gray-600">Chapter not found</p><Link to="/coaching" className="text-purple-400 text-sm mt-2 inline-block hover:underline">Back to Coaching</Link></div>
      </div>
    );
  }
          ...
          {chapter.lessons.map((lesson, i) => (
            <motion.div key={lesson.number} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
              <Link to={`/coaching/chapter/${chapterId}/lesson/${lesson.id}`}
                ...
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                  {lesson.number}
                </div>
