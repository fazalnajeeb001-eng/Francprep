...
import { Target, Clock, Award, Sparkles, Sun, Moon } from "lucide-react";
    ...
    <div className={`min-h-screen ${b} transition-colors duration-300`}>
      <div className="mx-auto max-w-5xl px-4 py-12">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold tracking-tight sm:text-5xl bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">Practice Exams</h1>
          <p className={`mx-auto mt-4 max-w-2xl text-lg ${txtSec}`}>Full-length mock exams for TCF Canada and TEF Canada with real-time scoring.</p>
        </motion.div>
        <div className="grid gap-6 md:grid-cols-2">
          {examTypes.map((ex, i) => (
            <motion.div key={ex.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
              <div className={`relative overflow-hidden rounded-2xl border ${card} backdrop-blur-lg p-6 transition-all hover:border-purple-500/50 hover:shadow-lg`}>
                <div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${ex.color}`} />
                <div className="flex items-start gap-4 mt-1">
                  <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${ex.color} flex items-center justify-center text-2xl shadow-lg`}>{ex.icon}</div>
                  <div className="flex-1">
                    <h2 className={`text-xl font-bold ${txtTitle}`}>{ex.flag} {ex.name}</h2>
                    <p className={`text-sm ${txtSec} mt-1`}>{ex.desc}</p>
                    ...
                    <div className="flex gap-3 mt-4">
                      <Link to="/exam/full" className="px-4 py-2 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 text-white text-xs font-semibold hover:opacity-90 shadow-lg shadow-purple-500/25">Full Exam</Link>
                      <Link to="/exam/practice" className="px-4 py-2 rounded-xl border border-[#1e2a4a] text-gray-300 text-xs font-semibold hover:bg-white/5">Practice</Link>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* Stats */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className={`mt-12 rounded-2xl border ${card} backdrop-blur-lg p-6`}>
          <h2 className={`text-lg font-semibold ${txtTitle} mb-4`}>📊 Your Exam Stats</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[{ label: "Exams Taken", value: "0", icon: <Target className="w-4 h-4 text-purple-400" /> }, { label: "Avg Score", value: "—", icon: <Award className="w-4 h-4 text-amber-400" /> }, { label: "Best Score", value: "—", icon: <Sparkles className="w-4 h-4 text-emerald-400" /> }, { label: "Study Time", value: "0h", icon: <Clock className="w-4 h-4 text-blue-400" /> }].map((stat) => (
              <div key={stat.label} className={`text-center p-3 rounded-xl ${dark ? "bg-[#070B17]" : "bg-gray-50"}`}>
                <div className="flex justify-center mb-1">{stat.icon}</div><p className={`text-lg font-bold ${txtTitle}`}>{stat.value}</p><p className="text-[10px] text-gray-500">{stat.label}</p>
              </div>
            ))}
          </div>
        </motion.div>
