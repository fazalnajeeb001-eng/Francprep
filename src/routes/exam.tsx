import { createFileRoute, useNavigate, Outlet, useMatch, Navigate, Link } from "@tanstack/react-router";
...
export function ExamHubPage() {
  ...
  const isFrenchTrack = activeLang === 'fr' || activeLang === 'fre' || activeLang === 'french';
    ...
    return (
      <div className={`min-h-screen ${bg} p-4 md:p-8 transition-colors duration-300`}>
        <div className="max-w-4xl mx-auto space-y-8 pb-20">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-4 border-b border-gray-200 dark:border-white/10">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-600 dark:text-purple-400 text-xs font-bold mb-2">
                ...
                <span>{branding.shortBrand} Authentic CBT Exam Practice Standard</span>
              </div>
          ...
          {/* Coming Soon Card */}
          <div className={`p-8 md:p-12 rounded-3xl border text-center space-y-6 shadow-2xl relative overflow-hidden ${
            dark ? "bg-gradient-to-br from-purple-950/40 via-[#101828] to-[#070B17] border-purple-500/30" : "bg-gradient-to-br from-purple-50 via-white to-purple-50 border-purple-200"
          }`}>
            <div className="w-20 h-20 rounded-3xl bg-gradient-to-tr from-purple-600 to-indigo-600 text-white flex items-center justify-center mx-auto shadow-xl shadow-purple-600/30">
              <Sparkles className="w-10 h-10" />
            </div>

            <div className="space-y-3 max-w-xl mx-auto">
              <span className="px-3.5 py-1 rounded-full bg-amber-500/20 text-amber-400 text-xs font-extrabold tracking-wider uppercase border border-amber-500/30 inline-block">
                🚧 CBT Simulator In Active Curriculum Production
              </span>
              <h2 className="text-2xl md:text-3xl font-extrabold">
                {branding.shortBrand} Full CBT Exam Simulator Coming Soon
              </h2>
              <p className={`text-xs md:text-sm ${txtSec} leading-relaxed`}>
                Our academic exam team is currently authoring official CBT practice suites for <strong>{branding.shortBrand} ({branding.examName})</strong>. In the meantime, you can build your core vocabulary, grammar, and speaking fluency using our interactive syllabus, flashcards, and AI speaking examiner!
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl mx-auto pt-4">
              <Link
                to="/learn"
                className="p-4 rounded-2xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs shadow-lg flex flex-col items-center gap-2 transition-all"
              >
                <BookOpen className="w-5 h-5" />
                <span>Explore {branding.languageName} Syllabus</span>
              </Link>
              <Link
                to="/flashcards"
                className={`p-4 rounded-2xl border font-bold text-xs flex flex-col items-center gap-2 transition-all ${
                  dark ? "bg-white/5 border-white/10 hover:bg-white/10 text-white" : "bg-white border-gray-200 hover:bg-gray-50 text-gray-900"
                }`}
              >
                <Sparkles className="w-5 h-5 text-amber-400" />
                <span>{branding.languageName} Flashcards</span>
              </Link>
              <Link
                to="/speaking"
                className={`p-4 rounded-2xl border font-bold text-xs flex flex-col items-center gap-2 transition-all ${
