          ...
          <div className="grid gap-3 sm:gap-4 mt-6 sm:mt-8 grid-cols-1 sm:grid-cols-2">
            {sections.map((section, i) => (
              <motion.div key={section} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                className="rounded-2xl dark:bg-[#101828]/80 bg-white/80 backdrop-blur-lg border dark:border-[#1e2a4a] border-gray-200 p-4 sm:p-5 flex items-center justify-between hover:border-purple-500/50 transition-all cursor-pointer">
