          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 h-full">
                ...
                {currentSection.type === "COMPREHENSION_ORALE" && (
                  <div className="p-3.5 sm:p-4 rounded-xl border space-y-3 bg-purple-50 border-purple-300 text-slate-950">
                    ...
                    {(currentQ as any).mainImageSvg ? (
                      <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-700 text-white space-y-2 shadow-md">
                        <div className="flex items-center justify-between text-xs font-bold text-amber-300">
                          <span>🎨 Dessin Vectoriel Officiel CBT N°{currentQ.questionNumber}</span>
                          <span className="text-[10px] text-slate-400 font-mono">Style Dessin au Trait (FEI CBT Standard)</span>
                        </div>
                        <div className="relative aspect-[16/10] w-full rounded-lg overflow-hidden border border-slate-800 bg-slate-900"
            ...
            <div className={`lg:col-span-5 p-4 sm:p-5 rounded-xl border ${cbtCard} shadow-sm space-y-5 flex flex-col justify-between`}>
              <div className="space-y-4">
                ...
                {/* Multiple Choice Options (Vector Line-Art Cards vs Text Options) */}
                <div className="space-y-2.5">
                  {(currentQ as any).optionImagesSvg && (currentQ as any).optionImagesSvg.length === 4 ? (
                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                      {currentQ.options.map((opt, idx) => {
                        const letter = String.fromCharCode(65 + idx); // A, B, C, D
                        const isChosen = selectedAnswers[currentQ.id] === idx;
                        const isLocked = mode === "PRACTICE" && checkedMap[currentQ.id];
                        const svgMarkup = (currentQ as any).optionImagesSvg[idx];
                          ...
                          >
                            ...
                            <div className={`px-3 py-2 flex items-center justify-between font-extrabold text-xs border-b ${
                              ...
                              <span className="flex items-center gap-1.5 font-mono">
                                ...
                                <span>Dessin {letter}</span>
                              </span>
                              {isChosen && <CheckCircle2 className="w-4 h-4 text-white" />}
                            </div>

                            {/* Vector Line Art Display */}
                            <div className="relative aspect-[4/3] w-full overflow-hidden bg-slate-900 flex items-center justify-center p-1"
