...
export function AuthenticCBTExamPage() {
  ...
  const handleRestartSessionClean = () => {
    ...
    if (paper?.id) {
      apiFetch(`/exam/active-session/${paper.id}`, { method: "DELETE" })
        .then((res) => res.json())
        .then((json) => {
          if (json?.sessionEpoch) {
            sessionEpochRef.current = json.sessionEpoch;
          }
        })
        .catch(() => {});
  ...
  const startSpeakingTaskSession = (idx: number) => {
    ...
    if (!hasPlayedIntroRef.current[task.id]) {
      ...
      let openingText = task.examinerPersona?.openingPromptFrench || (
    ...
    apiFetch(`/exam/active-session/${paper.id}?_t=${Date.now()}`, { cache: "no-store" })
      ...
      .then((json) => {
        ...
        if (json.success && json.activeSession) {
          if (json.activeSession.sessionEpoch) {
            sessionEpochRef.current = json.activeSession.sessionEpoch;
          }
  ...
  // Live Background Sync Pulse between active devices (Epoch-Guarded & Non-Destructive)
  useEffect(() => {
    if (!paper?.id || isSubmitted) return;
    const interval = setInterval(() => {
      ...
      apiFetch(`/exam/active-session/${paper.id}?_t=${Date.now()}`, { cache: "no-store" })
        ...
        .then((json) => {
          ...
          const remoteEpoch = json.activeSession.sessionEpoch || 1;

          // If another device clicked "Start New", respect the reset and clear stale in-memory state
          if (remoteEpoch > sessionEpochRef.current) {
            sessionEpochRef.current = remoteEpoch;
            setSelectedAnswers({});
            setFlaggedQuestions({});
            setWritingResponses({});
            setSpeakingTranscripts({});
          ...
          // Non-destructive live sync: merge remote answers without overwriting the candidate's active question
          if (json.activeSession.answers?.selectedAnswers) {
            const remoteAnswers = json.activeSession.answers.selectedAnswers;
            setSelectedAnswers((local) => {
              const currentQKey = currentQ?.id || `q_${currentQuestionIdx + 1}`;
              let hasChanges = false;
              const merged = { ...local };
              for (const [key, val] of Object.entries(remoteAnswers)) {
                // Never overwrite candidate's current actively viewed question
                if (key === currentQKey && local[key] !== undefined) continue;
                if (local[key] !== val) {
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                      ...
                      {(mode === "PRACTICE" || isSubmitted) && (
                        <button
                          ...
                          onClick={() => handleEvaluateWritingAI(task.id, task.prompt, textVal, task.sampleResponse, task.wordCountMin, task.wordCountMax, task.taskNumber || (activeWritingTaskIdx + 1))}
            ...
            <div className="bg-white dark:bg-slate-900 border-2 border-indigo-500/80 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4 font-sans text-slate-900 dark:text-slate-100 animate-in zoom-in-95 duration-150">
              <div className="flex items-center gap-3 border-b border-indigo-200 dark:border-indigo-900/60 pb-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-indigo-600 dark:text-indigo-400 font-mono font-bold uppercase">Session en cours détectée • Cloud Sync</p>
                </div>
              </div>

              <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed font-medium">
                Des réponses précédemment enregistrées ont été trouvées pour <strong>{paper?.title || 'cette épreuve'}</strong>. Souhaitez-vous continuer votre session ou recommencer un nouveau test ?
              </p>
                ...
                <div className="p-3 rounded-xl bg-indigo-50/80 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800/60 space-y-1.5 text-xs">
                  ...
                  {typeof cloudActiveSession.questionIndex === "number" && (
                    <div className="flex items-center justify-between text-[11px] font-semibold text-slate-600 dark:text-slate-400">
                      <span>Position de question:</span>
                      <span className="font-mono">Question #{cloudActiveSession.questionIndex + 1}</span>
                    </div>
                  )}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-end gap-2 pt-2">
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-2.5 rounded-xl border border-rose-200 dark:border-rose-900/60 bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 text-xs font-bold hover:bg-rose-100 dark:hover:bg-rose-900/60 transition-all cursor-pointer flex items-center justify-center gap-1.5"
                >
                  <RotateCcw className="w-3.5 h-3.5" />
                  <span>🚀 Recommencer à zéro / Start Fresh</span>
                </button>
                <button
                  type="button"
                ...
                >
                  <Sparkles className="w-3.5 h-3.5" />
                  <span>Reprendre la session / Resume →</span>
                </button>
              </div>
            </div>
          </div>
