    ...
    <div className={`min-h-screen ${bg} transition-colors duration-300`}>
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6 pb-20">
            ...
            {failedCount > 0 ? (
              <div className="flex flex-col gap-1.5">
                <button onClick={() => handleAutoFix('ai')} disabled={fixing}
                  className="px-3 py-1 bg-gradient-to-r from-purple-500 to-pink-500 hover:opacity-90 text-white text-[10px] font-bold rounded-xl shadow-md flex items-center gap-1 cursor-pointer">
                  {fixing ? <RefreshCw className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                  <span>🤖 AI Smart-Repair All</span>
                </button>
                <button onClick={() => handleAutoFix('quick')} disabled={fixing}
                  className="px-3 py-1 bg-gray-500/10 hover:bg-gray-500/20 text-gray-300 text-[9px] font-bold rounded-xl border border-gray-500/20 flex items-center justify-center gap-1 cursor-pointer">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-extrabold transition-all cursor-pointer ${
                selectedLevel === lvl
                  ...
                  : dark ? "bg-[#101828] text-gray-400 border border-[#1e2a4a] hover:text-white" : "bg-white text-slate-700 border border-slate-200 hover:bg-slate-50"
              }`}
            >
              {lvl} Level
            </button>
        ...
        {actionStatus.success && (
          <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs flex items-center gap-2 font-medium">
            <CheckCircle2 className="w-4 h-4 shrink-0" /> {actionStatus.success}
          </div>
        )}
        {actionStatus.error && (
          <div className="p-3.5 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-xs flex items-center gap-2 font-medium">
            <AlertTriangle className="w-4 h-4 shrink-0" /> {actionStatus.error}
          </div>
        )}
                ...
                <div className="border-b pb-3">
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    <button onClick={() => setPreviewLessonId(selectedReport.lessonId)}
                      className="px-3 py-1.5 bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/20 text-purple-400 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 cursor-pointer">
                      <Eye className="w-3.5 h-3.5" /> Preview & Edit Lesson
                    </button>

                    {selectedReport.status === 'fail' && (
                      <button
                        onClick={() => handleAutoFix('ai', [selectedReport.lessonId])}
                        disabled={fixing}
                        className="px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                      >
                        <Wrench className="w-3.5 h-3.5 text-emerald-400" /> {fixing ? "Repairing..." : "Fix This Specific Lesson"}
                      </button>
                    )}
                  </div>
                </div>
