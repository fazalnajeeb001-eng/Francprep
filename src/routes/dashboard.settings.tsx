...
import { Moon, Sun, Shield, Key, CreditCard } from "lucide-react";
      ...
      <div className="max-w-2xl mx-auto p-6 space-y-6 pb-20">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`${card} backdrop-blur-lg border rounded-2xl p-6 transition-colors`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className={`text-xs ${txtSec} block mb-1`} htmlFor="fn">First Name</label><input id="fn" type="text" defaultValue={user?.firstName} className={`w-full ${inputBg} rounded-xl px-3 py-2.5 text-sm ${dark ? "text-white" : "text-gray-900"} focus:outline-none focus:ring-2 focus:ring-purple-500 transition-colors`} /></div>
            <div><label className={`text-xs ${txtSec} block mb-1`} htmlFor="ln">Last Name</label><input id="ln" type="text" defaultValue={user?.lastName} className={`w-full ${inputBg} rounded-xl px-3 py-2.5 text-sm ${dark ? "text-white" : "text-gray-900"} focus:outline-none focus:ring-2 focus:ring-purple-500`} /></div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className={`${card} backdrop-blur-lg border rounded-2xl p-6 transition-colors`}>
          ...
          <div className="space-y-3">
            {["Current Password", "New Password", "Confirm New Password"].map((p) => (<input key={p} type="password" placeholder={p} aria-label={p} className={`w-full ${inputBg} rounded-xl px-3 py-2.5 text-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-colors ${dark ? "text-white" : "text-gray-900"}`} />))}
          </div>
