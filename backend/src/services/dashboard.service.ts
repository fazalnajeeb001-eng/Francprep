...
export class DashboardService {
  async getDashboard(userId: string): Promise<DashboardData> {
    ...
    const todayPlan = incompleteLessonDocs.map((l) => ({ id: l._id.toString(), title: l.title, type: (l.category ?? 'general') as string, completed: false }));
    ...
    return { user: { firstName: user.firstName, lastName: user.lastName, email: user.email, subscriptionTier: user.subscriptionTier, learningGoal: user.learningGoal || 'none', avatarUrl: user.avatarUrl || '', avatarFeatures: user.avatarFeatures || { gender: 'female', skinTone: '#F5C8A0', faceShape: 'oval', hairStyle: 'long', hairColor: '#3D2B1F', eyeColor: '#3D2B1F', eyeSize: 'medium', eyebrowStyle: 'natural', noseSize: 'medium', lipFullness: 'medium', facialHair: 'none', outfitStyle: 'hoodie', outfitColor: '#6A1B9A', accessory: 'none', earring: 'none', necklace: 'none' }, onboardingComplete: user.onboardingComplete || false, rpmGlbUrl: user.rpmGlbUrl || '' }, stats: { streak, xp, totalStudyTime, hearts }, levelProgress, overallProgress, lessonsCompleted: { completed: completedLessons, total: totalLessons }, vocabularyLearned, grammarMastered, averageScore, weeklyActivity, streakCalendar, todayPlan, continueLearning, recentAchievements };
  }
}
