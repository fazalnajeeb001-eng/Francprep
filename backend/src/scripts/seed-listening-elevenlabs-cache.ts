...
import { getExamRegistry, ExamSection } from '../../../src/lib/examSchema';

dotenv.config({ path: path.resolve(__dirname, '../../.env') });
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/francprep';

function getCanonicalHash(text: string, gender: string = 'female'): string {
  const normText = (text || '')
...
async function seed() {
  console.log('🎙️ Starting Multi-Account ElevenLabs Audio Cache Seeder...');
  ...
  console.log(`Loaded ${apiKeys.length} ElevenLabs API Key(s) for credit rotation.`);

  const femaleVoice = settings?.selectedElevenLabsFemaleVoice || 'EXAVITQu4vr4xnSDxMaL';
  const maleVoice = settings?.selectedElevenLabsMaleVoice || 'JBFqnCBsd6RMkjVDRZzb';

  let currentKeyIdx = 0;
  let currentKeyUsedChars = 0;
  const KEY_CREDIT_BUDGET = 9800;
  ...
  for (const paper of registry.slice(0, 10)) {
      ...
      for (const line of lines) {
        ...
        if (currentKeyUsedChars + cleanLine.length > KEY_CREDIT_BUDGET) {
          ...
          if (currentKeyIdx >= apiKeys.length) {
            ...
            console.warn('Progress is saved in MongoDB Atlas!');
            await mongoose.disconnect();
            return;
          }
        ...
        while (!success && currentKeyIdx < apiKeys.length) {
          ...
          try {
            ...
            let errDetail = err?.response?.data ? Buffer.from(err.response.data).toString('utf-8') : err?.message;
            console.warn(`[Key ${currentKeyIdx + 1} Error (${status})]: ${errDetail}`);

            if (status === 401 || status === 429 || status === 402 || errDetail?.includes('quota_exceeded')) {
